...
在安全机制上，每个产品配置签名密钥，客户端使用产品级公钥完成本地验签；每份许可证仍拥有独立的授权数据、设备绑定与生命周期状态。完整规则见 [许可证结构与验证](./license-token-structure.md)。
...
- **激活接口（Activate）**：客户端提交授权码和硬件指纹，服务端返回签名保护的许可证
...
| 接口 | 用途 |
| --- | --- |
| `POST /api/v1/activate` | 客户端激活：输入授权码，返回签名保护的许可证 |
