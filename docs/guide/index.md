...
::: tip 客户端接入推荐
如果您要把授权能力接入自己的软件，推荐直接打开 [AI 原生 API 快速接入（推荐）](/developer/ai-quickstart.md)。将页面中的协议提示词和适合您业务的模板交给 AI，即可在对接项目中使用原生 HTTP 接口完成接入，不要求安装 SDK。
:::
...
新项目推荐使用 [AI 原生 API 快速接入](/developer/ai-quickstart.md)：先复制统一的接口与许可证协议提示词，再选择符合业务的接入模板，让 AI 在您的对接项目中直接实现激活、本地验签、到期检查、试用、心跳、离线导入或许可证配置控制。

这种方式使用原生 HTTP 接口，不绑定编程语言，也不要求引入 SDK。协议页面已经给出接口输入输出、签名验证、安全边界和业务模板，便于 AI 生成符合当前项目技术栈的代码。

如果您的团队明确希望使用封装好的语言库，也可以选择 [客户端 SDK（可选）](./sdk.md)。
...
5. 如果您需要查阅许可证结构或接口索引，请查看 [许可证结构与验证（客户端要点）](./license-token-structure.md) 和 [接口文档](./api.md)
6. 只有在明确希望引入封装库时，再查看 [客户端 SDK（可选）](./sdk.md)
