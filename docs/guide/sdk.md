...
::: tip 新项目推荐
新项目优先使用 [AI 原生 API 快速接入（推荐）](/developer/ai-quickstart.md)。它通过统一协议提示词和业务模板，让 AI 在您的对接项目中直接调用原生 HTTP 接口，并把从产品页面取得的公钥内置为客户端固定信任根。
:::

> SDK 接入同样应从产品页面获取公钥，并通过 SDK 配置将其作为客户端固定信任根。
