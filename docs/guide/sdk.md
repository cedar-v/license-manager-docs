...
::: tip 新项目推荐
新项目优先使用 [AI 原生 API 快速接入（推荐）](/developer/ai-quickstart.md)。它通过统一协议提示词和业务模板，让 AI 在您的对接项目中直接调用原生 HTTP 接口，并按在线响应或离线交付方式处理产品公钥。
:::

> 在线首次激活可由 SDK 接收并缓存响应中的产品公钥；纯离线或固定信任根场景再显式配置公钥。
