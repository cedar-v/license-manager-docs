...
原生 API 灵活对接适合需要完全控制客户端授权逻辑的团队。您需要自行实现产品公钥内置、激活请求、许可证保存、本地验签、状态判断、心跳同步和异常处理。
...
| 产品编码 | 例如 `demo-app` |
| 产品公钥 | 在产品列表点击“获取公钥”，复制完整 PEM 公钥并内置到客户端 |
| 测试授权码 | 例如 `LIC-DEMO-XXXX-XXXX` |
| 客户端硬件指纹 | 用于绑定当前设备 |
| 本地保存目录 | 用于保存可变的 `license.dat`；产品公钥不保存在可写授权目录 |
...
开发阶段将产品公钥写入代码，或作为随程序发布的只读资源；首次激活成功后只需保存可变许可证：
...
::: warning 注意
正式客户端必须在首次在线激活前内置产品公钥，并始终使用它验证许可证。激活接口返回的 `public_key` 无需处理。
:::
