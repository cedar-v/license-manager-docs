...
原生 API 灵活对接适合需要完全控制客户端授权逻辑的团队。您需要自行实现激活请求、许可证保存、公钥保存、本地验签、状态判断、心跳同步和异常处理。
...
| 客户端硬件指纹 | 用于绑定当前设备 |
| 本地保存目录 | 用于保存 `license.dat` 和 `public_key.pem` |
...
::: warning 注意
当前激活接口会同时返回 `license_file` 和 `public_key`。客户端不需要在首次在线激活前单独下载公钥。
:::
