...
开发者中心面向需要把雪松授权云接入到自己软件中的工程团队。建议先阅读 [业务接入方式指导](./business-guidance.md) 确定授权流程，再选择 AI 原生 API、SDK 或手工原生 API 接入。

## 选择接入方式

| 选项 | 名称 | 适合谁 | 推荐程度 |
|------|------|--------|----------|
| 选项 1 | [AI 原生 API 快速接入](./ai-quickstart.md) | 希望让 AI 按完整协议和业务模板完成接入的团队 | 首推 |
| 选项 2 | [SDK 标准接入](./sdk/) | 使用官方 SDK，希望最少处理底层验签和心跳细节的团队 | 推荐 |
| 选项 3 | [原生 API 灵活对接](./quickstart.md) | 暂无 SDK 语言、已有授权框架、或需要完全自定义客户端逻辑的团队 | 高级 |

## 三种方式的差异

| 维度 | AI 原生 API 快速接入 | SDK 标准接入 | 原生 API 灵活对接 |
|------|-------------|--------------|------------------|
...
flowchart TD
    A[开始接入] --> B{希望怎么接入}
    B -->|让 AI 辅助改造项目| C[选项 1: AI 原生 API 快速接入]
...
| 任务 | 文档 |
|------|------|
| 选择授权业务方式 | [业务接入方式指导](./business-guidance.md) |
| 让 AI 使用原生 API 接入 | [AI 原生 API 快速接入（推荐）](./ai-quickstart.md) |
...
- 第一次接入请先阅读 [业务接入方式指导](./business-guidance.md)。
- 希望 AI 辅助改造对接项目，请使用 [AI 原生 API 快速接入（推荐）](./ai-quickstart.md)。
