...
SDK 可以减少底层接口和心跳处理工作。在线首次激活时，SDK 可接收并缓存接口返回的产品公钥；纯离线或需要固定信任根时，也可显式配置公钥。新项目还可以使用 AI 原生 API 接入，让 AI 按相同安全边界直接完成业务集成。
...
```mermaid
flowchart TD
    A["准备产品标识和激活码"] --> B["初始化 SDK"]
    B --> C["SDK 采集硬件指纹"]
    C --> D["SDK 读取本地许可证"]
    D --> E{"本地许可证有效?"}
    E -->|是| F["业务读取授权配置"]
    E -->|否| G["SDK 在线激活"]
    G --> H["使用响应公钥验签并缓存许可证与公钥"]
    H --> F
    F --> I["启动业务功能"]
    I --> J["SDK 后台心跳"]
    J --> K{"授权配置有更新?"}
